...
class OSEDiff_test(torch.nn.Module):
    def __init__(self, args):
        ...
        self.args.pretrained_model_name_or_path = "CompVis/stable-diffusion-v1-4"
    ...
    def load_ckpt(self, model):
        ...
        """
        # Load parameters with shape check
        for n, p in self.unet.named_parameters():
            if "lora" in n or "conv_in" in n:
                if n in model["state_dict_unet"]:
                    ckpt_tensor = model["state_dict_unet"][n]
                    try:
                        if p.shape == ckpt_tensor.shape:
                            p.data.copy_(ckpt_tensor)
                        elif ckpt_tensor.ndim == 2 and p.ndim == 4 and p.shape[2:] == (1, 1):
                            # reshape 2D linear to 1x1 conv
                            reshaped = ckpt_tensor.view(p.shape)
                            p.data.copy_(reshaped)
                            print(f"✅ Reshaped and loaded '{n}': {ckpt_tensor.shape} → {p.shape}")
                        else:
                            print(f"⚠️ Skipping '{n}': shape mismatch {p.shape} (model) vs {ckpt_tensor.shape} (ckpt)")
